...
df = pd.read_csv("current_csv/osmosss_block_data_temp.csv", parse_dates=['time'])
...
plt.savefig("transaction_distribution.png")  # ヒストグラムを画像ファイルとして保存
plt.close()

# ブロック生成時間の推移
plt.figure(figsize=(8, 4))
plt.plot(df['height'], df['block_interval'], marker='o', linestyle='-')
