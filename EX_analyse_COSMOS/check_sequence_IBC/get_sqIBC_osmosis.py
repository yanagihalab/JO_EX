...
def get_latest_block_height_and_time():
    status_url = "https://osmosis-rpc.publicnode.com/status"
...
def save_csv(start_height, end_height, directory):
    ...
    if packet_delays:
        ...
        csv_filename = os.path.join(directory, f"GG_OSMO_ibc_sq_pk{start_height}-{end_height}.csv")
        df.to_csv(csv_filename, index=False)
        print(f"CSV file saved: {csv_filename}")

    if recv_packets:
        df_recv = pd.DataFrame(recv_packets)
        recv_filename = os.path.join(directory, f"GG_OSMO_ibc_sq_rv_{start_height}-{end_height}.csv")
