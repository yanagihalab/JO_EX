...
def analyze_long_blocks(threshold_sec: float):
    ...
    plt.title(f"Top Proposers: Ratio of Blocks ≥ {threshold_sec:.1f}s")
    plt.xlabel("Proposer Address (prefix)")
    plt.ylabel(f"% of Blocks ≥ {threshold_sec:.1f}s")
    plt.tight_layout()
    filename = f"over_{threshold_sec:.1f}s_ratio_bar.png".replace('.', '_')
